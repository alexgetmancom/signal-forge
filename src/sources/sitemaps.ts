 ...
 */
const SITEMAPS: readonly { url: string; maker: string }[] = [
...
const MODEL_PAGE =
  /^(?:introducing-|previewing-|announcing-)?(?:gpt|o\d|sora|codex|gemini|gemma|veo|imagen|lyria|claude|opus|sonnet|haiku|mythos|fable)(?:-[a-z]+)*-\d[a-z0-9-]*$/;
...
export async function collectLabSitemap(
  ...
  for (const sitemap of sitemaps) {
    ...
    if (!xml.includes("<loc>")) throw new Error(`${sitemap.url} is not a sitemap`);
    for (const url of modelPages(xml)) records.set(url, { id: url, name: url.split("/").at(-1) ?? url, maker, url });
  }
