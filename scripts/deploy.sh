...
ssh "$DEPLOY_HOST" "cd $REMOTE_DIR && chmod 600 .env signal-forge.json && docker compose stop app && docker compose build app && docker compose run --rm --no-deps app bun dist/scripts/backfill-leaderboards.js && docker compose up -d --wait --wait-timeout 180"
