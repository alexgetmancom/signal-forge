...
| Owner decision | Vertex: a service account for `vertex-quotas` and `vertex-model-garden`. | On 2026-09-17 the Cloud Quotas API answered `base_model:grok-4.7` (47,000 input, 4,000 output tokens and 3 requests a minute) for project `tasknotes-486221` while Model Garden's newest xAI entry was `grok-4.6`, both read from Cloud Shell. Both sources are registered and answer 401 without a token. Done when a service account in that project holding `roles/cloudquotas.viewer` and `roles/aiplatform.viewer` has its JSON key in `GOOGLE_CLOUD_SERVICE_ACCOUNT` on production and both collect. Bedrock and Azure AI Foundry still need an account nobody has created. |
